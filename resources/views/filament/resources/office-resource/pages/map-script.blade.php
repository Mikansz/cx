...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initMapSearch() {
        ...
        if (!mapContainer && attempts < maxAttempts) {
            attempts++;
    ...
    function updateMapPosition(lat, lng) {
        ...
        if (mapContainers.length > 0) {
            ...
            if (locationInput) {
                const locationData = JSON.stringify({lat: parseFloat(lat), lng: parseFloat(lng)});
    ...
    // Initialize after a delay to ensure map is loaded
    setTimeout(initMapSearch, 1000);
});
</script>